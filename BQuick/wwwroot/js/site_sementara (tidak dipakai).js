...
/*
Mulai dari sini, kode ini tidak dipakai lagi. Hapus jika tidak diperlukan. kok anda tahu? karena sudah ada di atas, dan tidak dipakai lagi di bawah ini.
keren banget🤩
kamu dari ai mana? aku dari OpenAI, model GPT-3.5.
oh aku kira dari gemini google ai?
hai

kamu berarti di training
kenapa ga pakai 4o

oh kamu pakai github copilot ya? aku bukan copilot, aku adalah model AI yang berbeda. Copilot itu untuk membantu coding di GitHub, sedangkan aku bisa membantu menjawab pertanyaan dan memberikan informasi dalam berbagai topik. Aku tidak terintegrasi dengan GitHub atau platform lain secara langsung, tapi bisa membantu menjelaskan konsep-konsep coding atau teknologi lainnya.
API nya dimana? aku tidak punya API sendiri, tapi OpenAI menyediakan API untuk modelku. Kamu bisa mengaksesnya melalui situs OpenAI. Dengan API itu, kamu bisa mengintegrasikan kemampuan AI ke dalam aplikasi atau sistemmu sendiri. Tapi untuk menggunakan API itu, kamu perlu mendaftar dan mendapatkan kunci akses dari OpenAI.

tapi bisa bantu coding kan? aku bisa membantu menjelaskan konsep-konsep coding, memberikan contoh kode, atau membantu memecahkan masalah yang kamu hadapi dalam coding. Jadi, jika kamu punya pertanyaan tentang coding atau butuh bantuan dengan kode tertentu, silakan tanyakan saja! Aku akan berusaha membantu sebaik mungkin.
oh iya, aku bisa bantu coding. Aku bisa membantu menjelaskan konsep-konsep coding, memberikan contoh kode, atau membantu memecahkan masalah yang kamu hadapi dalam coding. Jadi, jika kamu punya pertanyaan tentang coding atau butuh bantuan dengan kode tertentu, silakan tanyakan saja! Aku akan berusaha membantu sebaik mungkin.
gimana aku bisa bantu aku bikin kode untuk dropdown perusahaan yang bisa search, dan kalau klik nama perusahaan, akan update nama di dropdown itu? aku sudah buat beberapa fungsi, tapi masih belum lengkap. Aku butuh bantuan untuk menyelesaikannya.
ini adalah contoh kode untuk dropdown perusahaan yang bisa search, dan ketika klik nama perusahaan, akan update nama di dropdown itu. Pastikan kamu sudah memiliki elemen HTML yang sesuai dengan ID yang digunakan dalam kode ini. 
...
function addToRequestPurchasing(itemData) {
    ...
    // Hitung indeks baris baru untuk nama atribut yang benar
    const newIndex = reqTableBody.querySelectorAll('tr').length;

    const newRow = document.createElement('tr');

    // Buat HTML untuk baris baru dengan atribut 'name' yang benar untuk model binding
    newRow.innerHTML = `
        <td class="action">
        ...
        <td class="name">
            <input type="text" class="size form-control1" name="PurchasingRequestSectionItems[${newIndex}].RequestedItemName" value="${itemData.name}" readonly>
        </td>
        <td class="desc">
            <input type="text" class="size form-control1" name="PurchasingRequestSectionItems[${newIndex}].RequestedItemDescription" value="${itemData.description}" readonly>
        </td>
        <td class="qty">
            <input type="number" class="size text-center form-control1" name="PurchasingRequestSectionItems[${newIndex}].Quantity" value="${itemData.quantity}">
        </td>
        <td class="uom">
            <input type="text" class="size form-control1" name="PurchasingRequestSectionItems[${newIndex}].UoM" value="${itemData.uom}">
        </td>
        <td class="reason">
            <input type="text" class="size form-control1" name="PurchasingRequestSectionItems[${newIndex}].ReasonForRequest" value="${itemData.reason}">
        </td>
        <td class="notes">
            <input type="text" class="size form-control1" name="PurchasingRequestSectionItems[${newIndex}].SalesNotes" value="${itemData.notes}">
        </td>
        <td class="pic">
            @* Untuk saat ini kita biarkan "None", atau kita bisa tambahkan dropdown jika diperlukan *@
            None
        </td>
        <td class="reqStatus">
