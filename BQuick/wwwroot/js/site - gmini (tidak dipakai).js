...
function disableConfirmPicFormFields() {
    const popup = document.querySelector('.confirm-pic-form-pop-up');
    if (!popup) return;

    popup.querySelectorAll('input, select, textarea').forEach(el => {
        if (el.closest('.change-pic-form')) return;
        el.disabled = true;
    });

    popup.querySelectorAll('.wrapp').forEach(wrapper => {
        if (wrapper.closest('.change-pic-form')) return;
        wrapper.classList.add('disabled-by-script');
        wrapper.setAttribute('tabindex', '-1');
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const togglerMobile = document.querySelector("#sidebar-mobile-icon");
    ...
    notifTabs.forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            document.querySelector(`.notifications-panel[data-panel="${this.dataset.tab}"]`).classList.add('active');
        });
    });
    ...
    document.addEventListener('click', function (e) {
        const openBtn = e.target.closest('.open-rfq-info-btn');
        if (openBtn) {
            const popup = document.querySelector('.confirm-pic-form-pop-up');
            if (popup) {
                window.currentChangePicRow = openBtn.closest('tr');
                popup.classList.add('active');
                document.body.classList.add('pop-up-active');
                popup.scrollTop = 0;
                disableConfirmPicFormFields();

                const changePicForm = popup.querySelector('.change-pic-form');
                if (changePicForm) {
                    const inputPICs = [];
                    document.querySelectorAll('#surveyTableBody .survey-pic-dropdown .select-btn input[type="text"]').forEach(input => {
                        const val = input.value.trim();
                        if (val) {
                            val.split(',').forEach(name => {
                                const clean = name.trim();
                                if (clean && !inputPICs.includes(clean)) inputPICs.push(clean);
                            });
                        }
                    });

                    const dropdown = changePicForm.querySelector('.survey-pic-dropdown');
                    if (dropdown) {
                        dropdown.querySelectorAll('.survey-list-checkboxes').forEach(cb => {
                            cb.checked = inputPICs.includes(cb.value);
                        });

                        const input = dropdown.querySelector('.select-btn input[type="text"]');
                        if (input) {
                            input.value = inputPICs.join(', ');
                        }

                        setupMultiSelectDropdown(dropdown);
                    }

                    changePicForm.style.display = 'none';
                }
            }
        }

        const closeBtn = e.target.closest('#change-pic-form-close-btn');
        if (closeBtn) {
            const popup = document.querySelector('.confirm-pic-form-pop-up');
            if (popup) {
                popup.classList.remove('active');
        ...
        const confirmBtn = e.target.closest('.change-pic-confirm-btn');
        if (confirmBtn) {
            const popup = document.querySelector('.confirm-pic-form-pop-up');
            if (popup) {
                popup.classList.remove('active');
            ...
            const row = window.currentChangePicRow;
            if (row) {
                const actionCell = row.cells[7];
                if (actionCell) {
                    actionCell.innerHTML = `<div class="text-center">Assign PIC</div>`;
                }
                const statusCell = row.cells[8];
                if (statusCell) {
                    statusCell.innerHTML = `
                        <div class="d-flex justify-content-center align-items-center flex-column gap-1">
                            <span id="completed" class="rounded-pill status">Completed</span>
                        </div>
                    `;
                }
            }
            window.currentChangePicRow = null;
        }
    });

    const changePicBtn = document.getElementById('change-pic-btn');
    const changePicForm = document.querySelector('.change-pic-form');
    if (changePicBtn && changePicForm) {
        changePicBtn.addEventListener('click', function () {
            if (changePicForm.style.display === 'none' || changePicForm.style.display === '') {
                changePicForm.style.display = 'block';
            } else {
                changePicForm.style.display = 'none';
            }
        });
    ...
    if (addRowBtn && itemTableBody) {
        addRowBtn.addEventListener('click', () => {
            ...
            newRow.innerHTML = `
                ...
                <td class="qty"><input type='number' class='size text-center form-control1'></td>
                <td class="uom"><input type='text' class='size form-control1'></td>
                <td class="budget"><input type='number' class='size form-control1'></td>
                <td class="leadtime"><input type='text' class='size form-control1'></td>
                <td class="delete"><button type="button" class="btn btn-remove-row"><i class='bx bx-trash'></i></button>
            `;
